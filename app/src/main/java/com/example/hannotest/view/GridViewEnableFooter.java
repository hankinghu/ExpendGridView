 ...
 * See {@link GridViewEnableFooter#addHeaderView(View, Object, boolean)}
 * See {@link GridViewEnableFooter#addFooterView(View, Object, boolean)}
 */
public class GridViewEnableFooter extends GridView {
    ...
    public GridViewEnableFooter(Context context, AttributeSet attrs) {
        super(context, attrs);
        initHeaderGridView();
    }

    public GridViewEnableFooter(Context context, AttributeSet attrs, int defStyle) {
    ...
    /**
     * 设置折叠的行数，默认-1，如果不设置默认不折叠
     */
    public void setFoldNum(int foldNm) {
        this.foldNm = foldNm;
    }
    ...
    public int getRowHeight() {
        ...
        LayoutParams p = new LayoutParams(view.getLayoutParams().width, view.getLayoutParams().width, 0);
    ...
    /**
     * 对于传入的adapter进行一层包装，使得可以实现折叠的功能
     */
    private class FoldViewGridAdapter implements WrapperListAdapter {
        private final ListAdapter adapter;
        private final int fn;

        public FoldViewGridAdapter(ListAdapter adapter, int foldNm) {
    ...
    public void notifyDataChanged() {
        if (getAdapter() != null && getAdapter() instanceof HeaderViewGridAdapter) {
